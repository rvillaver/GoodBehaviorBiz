...
its index line. **Don't leave two memories disagreeing, and don't keep a corrected fact alive as if it were still
true.** If you are unsure whether it's superseded, **leave it.** An outdated memory you can still read beats a lost one.
...
**Don't record what the repo already states** (code structure, past fixes, git history, AGENTS.md). If asked to
"remember" something obvious, ask what was *non-obvious* about it and record that.
...
(what future projects should do). **Strip names, paths, URLs, customers, credentials**, and any detail not needed to
teach the rule. If the promotion is accepted upstream, add the commit/PR reference to the memory so future updates
can trace it.
