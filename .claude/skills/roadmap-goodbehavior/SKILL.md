...
- If an item turns out 10× bigger or mostly-invisible than peers, **flag it and right-size it** (split, or backlog the
  heavy part). Surface the tradeoff and recommend, rather than silently grinding; the user decides.
...
- **Supersede stale "all done" claims explicitly.** Keep old status as history so it can't mislead, **in the archive,
...
2. **Leave one digest line in the plan**, under a `## Closed` heading: what the phase delivered, when, and a link to
   the evidence file. One line, not a collapsed table or a struck-through copy.
