...
- **Cross-checked against the reference**, element by element. The gap is the *delta* (what the reference has that we
  don't, or differs), **stated concretely rather than paraphrased** ("looks different" is not a gap).
...
coherence**. **Run the lenses as separate passes**, or parallel sub-agents each blind to the others, then reconcile.
A contradiction between two lenses (one says "done," another "unreachable") is itself a P0/P1 finding.
...
Keep an index (`docs/audit/00-index.md`) with batch status and the enumerated real surface, meaning the actual units for
the profile (screens/endpoints/components; datasets/queries; sources/claims; sections/slides), so coverage is auditable.
...
`/roadmap-goodbehavior`. **The audit documents gaps; it does not build them. Don't start fixing mid-audit.**
