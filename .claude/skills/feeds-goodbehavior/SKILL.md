...
only** — a match is audited, never blocks or asks. They augment the guard's hardcoded hard-shape/zone logic,
...
- **`secrets`** — [gitleaks](https://github.com/gitleaks/gitleaks)'s rule set (credentials/token patterns),
  ~100KB, ~220 rules, checked against command text (not file contents).
...
- `update` never leaves a feed half-written: a fetch that fails, or content that fails to compile, or compiles
  to zero rules, leaves whatever was already installed untouched and reports `feed_update_failed`.
- A successful update that changes content rotates the previous version to `previous/` — `rollback` swaps
  current and previous (calling it twice toggles back and forth, it is not a one-way undo).
...
user, translate — "3 feeds installed, last updated 2 days ago, 342 rules total" reads better than the raw JSON.
For what a feed actually *caught*, that's `/report-goodbehavior`'s job (it reads the audit trail, which
includes feed hits — the shape name for a feed hit is `feed:<name>:<rule-id>`), not this skill's.
...
through a Go-RE2-to-JS translation (a handful of rules that use syntax JS can't represent are skipped, counted,
never silently dropped), and Sigma rules that need process context this guard can't see (parent process, user,
